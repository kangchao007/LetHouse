...
<script language="JavaScript">
    ...
    function SaveDialog(){
        ...
        $('#saveDialogForm').form('submit', {
            url:"/admin/insertDistrict",  //提交的服务器地址
    ...
    function SaveDialog2() {
        $('#upDialogForm').form('submit', {
            url:"/admin/updateDistrict",  //提交的服务器地址
            success:function(data){ //成功的回调函数
                //data接收服务器返回值json字符串（不是json对象）
                var obj=$.parseJSON(data);
                if(obj.result>0){
    ...
    function DeleteByFruitName() {
        ...
        $.messager.confirm("系统提示", "你确定要删除<font color=red> " + SelectRows.length + " </font>条数据吗?", function(){
            //发送异步请求到服务器
            $.post("deleteMoreDistrict",{ids:delValue.join(",")},function (data) {
                if (data.result>0){
    ...
    function delSingle(id) {
        ...
        function openStreetDialog(obj){  //obj接收区域编号
            ...
            $('#streetDg').datagrid({
                title:">>>>街道信息",
                url:'/admin/selectStreetByDid?districtId='+obj,
                fitColumns: true,
